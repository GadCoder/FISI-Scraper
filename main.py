...
def check_if_txt_time_exists():
    txt_path = os.path.join(FILES_PATH, "last_update.txt")
...
def update_news():
    ...
    if check_main_posts():
        ...
        telegram_bot_send_message(
            f"{get_current_time()}: No hay noticias principales nuevas ☹️",
            isPersonal=True,
        )
    ...
    if check_under_posts():
        ...
        telegram_bot_send_message(
            f"{get_current_time()}: No hay noticias principales secundarias ☹️",
            isPersonal=True,
        )
